...
curl -sS -o /dev/null -X DELETE -H "$AUTH" "${API}/git/refs/tags/${TAG}" || true
...
rid=$(curl -sS -X POST -H "$AUTH" -H "Content-Type: application/json" \
  -d "$(jq -n --arg t "$TAG" --arg c "$SHA" --arg n "$title" --arg b "$body" \
        '{tag_name:$t, target_commitish:$c, name:$n, body:$b, draft:false, prerelease:false}')" \
...
url=$(curl -sS -X POST -H "$AUTH" \
  ...
  --data-binary "@${EXE}" \
  "${UPLOAD_API}/releases/${rid}/assets?name=${NAME}" | jq -r '.browser_download_url // empty')
...
upload_asset() {
  ...
  curl -sS -X POST -H "$AUTH" \
    -H "Content-Type: application/octet-stream" \
    --data-binary "@${file}" \
    "${UPLOAD_API}/releases/${rid}/assets?name=${name}" | jq -e '.browser_download_url != null' >/dev/null
}
