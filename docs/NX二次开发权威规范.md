...
- 统一风格：白色底；紫味深灰 `#433A5E` 清晰描边；对象顶面使用浅紫白 `#EDE6FC`、侧面使用
  浅紫灰 `#B6A6E0`；被操作的功能特征使用亮紫 `#9061F0`，导出、定位、尺寸或方向等动作使用
  强调紫 `#7C57E5`。采用纯色平涂技术图标构图，主体居中、留出边距，在 32 像素下仍能辨认；完整元件和
  用色方法见 `docs/design/icon-design-language.md`，其文件格式、命名和发布规则不得与本规范冲突。
...
CI 按 `plugins.json` 克隆、收集和校验插件，生成单文件安装器，并同时发布资源清单及逐项资源附件到滚动 `latest`。Release 名称必须符合安装器解析契约；远端版本不高于本地版本时不得提示或降级。
...
- NX 检查更新先按资源清单逐项比较哈希并热更新；仅当 `installer_version` 高于本地安装器版本时，才询问是否下载完整安装器。历史清单没有 `installer_version` 时回退旧版行为，避免漏报安装器更新。
...
- NX 内检查更新先下载清单，仅下载哈希发生变化的 DLL/DLX/DAT/菜单/图标等资源；完整安装器 exe 作为独立的可选更新路径。
